...
export const DelayedEffectSchema = z.object({ afterMonths:z.number().int().positive(), effects:EffectsSchema, source:z.string().optional(), prose:z.string().optional() });
export const ChoiceSchema = z.object({ id:z.string(), label:z.string(), effectSummary:z.string(), effects:EffectsSchema, delayed:z.array(DelayedEffectSchema).default([]) });
export const ActivitySchema = z.object({ id:z.string(), name:z.string(), description:z.string(), effects:EffectsSchema, delayed:z.array(DelayedEffectSchema).default([]) });
